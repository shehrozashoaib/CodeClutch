...
def _supports_ansi_tools() -> bool:
    return sys.stdout.isatty() and os.environ.get("TERM", "") != "dumb"


def _style_tools(text: str, *codes: str) -> str:
    if not _supports_ansi_tools():
        return text
    return "".join(codes) + text + RESET


def _print_tools_panel(title: str, tone: str = "info") -> None:
    colors = {"info": BLUE, "success": GREEN, "error": RED}
    color = colors.get(tone, BLUE)
    rule = _style_tools("─" * 80, color) if _supports_ansi_tools() else ("-" * 80)
    label = _style_tools(title, color, BOLD) if _supports_ansi_tools() else title
    print("\n" + rule)
    print(label)
    print(rule)
    ...
    if normalized in {"pip", "pip3"}:
        ...
        if conda_exe:
            ...
            if len(parts) > 1 and parts[1] == "install":
                ...
                has_yes = any(flag in parts for flag in ["-y", "--yes"])
                if not has_target:
                    user_env_dir = os.environ.get("USER_ENV_DIR", "/workspace/.venvs/user")
...
def read_file(
    ...
    try:
        ...
        if ext == ".json" or (file_type and "JSON" in file_type):
            ...
            try:
                result["content"] = json.loads(raw_json_text)
                result["file_type"] = "JSON"
            except json.JSONDecodeError as exc:
                lines = raw_json_text.split("\n")
                numbered = [f"{i+1:5d} | {line}" for i, line in enumerate(lines)]
                result["content"] = "\n".join(numbered)
                result["file_type"] = f"JSON (raw text; parse error on line {exc.lineno})"
                result["parse_error"] = f"{exc.msg} at line {exc.lineno} column {exc.colno}"
            result["success"] = True
...
def run_script(script_path: str, args: List[str] | None = None) -> Dict[str, Any]:
    ...
    _print_tools_panel(f"Run Script · {script_path}", tone="info")
    python_bin = _resolve_user_python()
    ...
    try:
        ...
        if result.returncode == 0:
            _print_tools_panel(f"Run Complete · exit={result.returncode}", tone="success")
        else:
            _print_tools_panel(f"Run Failed · exit={result.returncode}", tone="error")
    except FileNotFoundError:
