...
def _safe_ast_parse(source: str, filename: str = "<unknown>") -> ast.AST:
...
def _print_compact_resource_summary(discovered: Dict[str, List[str]]) -> None:
    non_empty = [(key, len(values)) for key, values in discovered.items() if values]
    if not non_empty:
        print("\nResource discovery: no reusable configs or datasets found.")
        return
    summary = ", ".join(f"{key}={count}" for key, count in non_empty)
    print(f"\nMinimal-input resources: {summary}")

def _print_minimal_plan_summary(func_key: str, metrics: Dict[str, int], helper_name: str) -> None:
    summary = ", ".join(f"{key}={value}" for key, value in metrics.items())
    print(f"  - {func_key}: {summary}; helper={helper_name}")
    ...
    def parse_file(self, file_path: str) -> ast.AST:
        ...
        if file_path not in self.file_asts:
            with open(file_path, "r", encoding="utf-8") as f:
                self.file_asts[file_path] = _safe_ast_parse(f.read(), file_path)
...
class ExistingResourceDiscovery:
    ...
    def discover_all_resources(self) -> Dict[str, List]:
        ...
        print("\nDiscovering reusable project resources...")
        ...
        for pattern in [
            ...
            for file in self.project_dir.glob(pattern):
                if file.is_file() and "test" not in str(file).lower() and not _should_ignore_discovery_path(file):
                    if file.suffix == ".json":
        ...
        for pattern in ["**/*.pth", "**/*.pt", "**/*.ckpt", "**/checkpoints/**"]:
            for file in self.project_dir.glob(pattern):
                if file.is_file() and not _should_ignore_discovery_path(file):
        ...
        _print_compact_resource_summary(self.discovered)
        ...
        for script_path in self.discovered["example_scripts"]:
            try:
                ...
                if func_name in content:
                    tree = _safe_ast_parse(content, script_path if "script_path" in locals() else file_path)
...
class SmartParameterResolver:
    ...
    def resolve_derived_parameters(
        ...
        if split_info["found"]:
            print(f"     Dataset split info found: {split_info['ratios']}")
...
class EnhancedParameterResolver(SmartParameterResolver):
    ...
    def resolve_config_based_parameters(
        ...
        print("\n   Analyzing config-backed parameters...")
        ...
        print(f"     Config attributes referenced: {len(config_attrs_used)}")
        ...
        if config_file and Path(config_file).exists():
            ...
            print(f"     Config class attributes available: {len(config_class_attrs)}")
...
def generate_complete_minimal_inputs(
    ...
    for func in expensive_functions:
        ...
        _print_minimal_plan_summary(
            func_key,
            {
                "critical_minimal": len(critical_params.get("critical_minimal", [])),
                "critical_default": len(critical_params.get("critical_default", [])),
                "config_values": len(found_values),
                "orchestrator_defaults": len(orchestrator_defaults),
            },
            safe_name,
        )
...
def generate_complete_minimal_inputs_v2(
    ...
    for func in expensive_functions:
        ...
        changed_attrs = [
            (attr, info)
            for attr, info in all_resolved["config_attributes"].items()
            if info["minimal_value"] != info["default"]
        ]
        ...
        _print_minimal_plan_summary(
            func_key,
            {
                "direct_minimal": len(all_resolved["critical_minimal"]),
                "config_attrs": len(all_resolved["config_attributes"]),
