...
def read_file(
    file_path: str, file_type: Optional[str] = None, error_line: Optional[int] = None
) -> Dict[str, Any]:
        ...
        if ext == ".json" or (file_type and "JSON" in file_type):
            ...
            for encoding in encodings:
                try:
                    ...
                    start_val = 0
                    end_val = len(numbered)

                    if error_line is not None and error_line > 0 and len(numbered) > 100:
                        print("using smaller portion")
                        
                        # Convert to 0-indexed
                        err_idx = error_line - 1
                        
                        # Start: search upward for function start, but not more than 50 lines
                        start_val = max(0, err_idx - 50)
                        for i in range(err_idx, start_val - 1, -1):
                            if re.search(r"def\s+\w+\s*\(", numbered[i]):
                                start_val = i
                                break
                            
                        # End: at least 20 lines AFTER error_line, or next function
                        end_val = min(len(numbered), err_idx + 30)
                        for i in range(err_idx + 1, end_val):
                            if re.search(r"^def\s+\w+\s*\(", numbered[i].split("|")[-1].strip()):
                                end_val = i
                                break
                            
                        # CRITICAL: Ensure error_line is ALWAYS included
                        if err_idx >= end_val:
                            end_val = min(len(numbered), err_idx + 20)
                        if err_idx < start_val:
                            start_val = max(0, err_idx - 10)
                        
                        result["content"] = "\n".join(numbered[start_val:end_val])
                    else:
                        result["content"] = "\n".join(numbered)
...
def error_analyzer(error: str) -> Dict[str, Any]:
    """Analyze Python traceback to locate last error and file/line context."""

    counts = error.count("File")
    my_pattern = r'File "([^"]+)"'
    match = re.findall(my_pattern, error)
    current_cwd = os.getcwd()

    for i in range(len(match)):
        match[i] = match[i].replace(current_cwd, "")

    error_flow = match[0] if match else ""
    for file in match[1:]:
        if not error_flow.endswith(file):
            error_flow += f"->{file}"

    single_file_error = len(match) == len(set(match))

    parsed = re.split("File", error)
    last_error = "File " + parsed[-1]
    ...
    # Check for ModuleNotFoundError anywhere in the error, not just last_line
    # Check for ModuleNotFoundError anywhere in the error
    if "ModuleNotFoundError" in error:
        module_pattern = r"No module named ['\"]([^'\"]+)['\"]"
        module_match = re.search(module_pattern, error)
        if module_match:
            missing_module = module_match.group(1)
            # Extract just the top-level module name (e.g., "alignn" from "alignn.data")
            top_level_module = missing_module.split(".")[0]
            
            # Check if it's a local file or a library
            files = list_files.invoke({})
            is_local_file = False
            for fname in files.keys():
                # Must be EXACT match: "alignn.py" or "alignn/" directory
                # NOT partial match like "train_alignn.py"
    ...
    line_parsed_match = re.search(r"line\s+(\d+)", last_error)
    error_line = int(line_parsed_match.group(1)) - 1 if line_parsed_match else 0
