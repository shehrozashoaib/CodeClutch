...
def _build_capture_wrapper(func_name: str, cache_file: str, indent: int = 0) -> str:
    ...
    return (
        ...
        f"{body_ind}import pickle\n"
        f"{body_ind}cache_path = '{cache_file}'\n"
        f"{body_ind}# If cache exists, return cached value (MOCK mode)\n"
        f"{body_ind}if os.path.exists(cache_path):\n"
        f"{body_ind2}print(f'[MOCK] {func_name}() - returning cached result')\n"
        f"{body_ind2}with open(cache_path, 'rb') as f:\n"
        f"{body_ind2}    return pickle.load(f)\n"
        f"{body_ind}# First run: execute original, cache result\n"
        f"{body_ind}print(f'[CAPTURE] {func_name}() - executing and caching result...')\n"
        f"{body_ind}result = _original_{func_name}(*args, **kwargs)\n"
        f"{body_ind}# Save result to cache\n"
        f"{body_ind}os.makedirs(os.path.dirname(cache_path) or '.', exist_ok=True)\n"
        f"{body_ind}with open(cache_path, 'wb') as f:\n"
        f"{body_ind2}pickle.dump(result, f)\n"
        f"{body_ind}print(f'[CAPTURE] {func_name}() - result cached')\n"
        f"{body_ind}return result\n"
    )
